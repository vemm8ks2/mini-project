...
from app.util.저장_경로_생성 import 저장_경로_생성
...
def process_method():
    ...
    if 재무제표종류 and 항목코드 and 결산기준일:
        ...
        선_차트.write_image(f"{저장_경로}/{회사명}--선--그래프.jpeg")
        막대_차트.write_image(f"{저장_경로}/{회사명}--막대--그래프.jpeg")

        저장종목 = 종목[["결산기준일", "재무제표종류", "항목명", "항목코드", "회사명", "당기"]]
        저장종목.to_csv(f"{저장_경로}/{회사명}--데이터프레임.csv", index=False)

        return render_template(
            "result.html",
            결과=종목.to_dict(orient="records"),
            선_그래프=선_차트.to_html(full_html=False),
            막대_그래프=막대_차트.to_html(full_html=False)
        )
